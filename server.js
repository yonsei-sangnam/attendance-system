  ...
  <body style="display:flex;align-items:center;justify-content:center;min-height:100vh;padding:20px;background:#003776;">
    ...
    <script>
      ...
      async function passkeyAuth() {
        ...
        try {
          ...
          if (err.name === 'NotAllowedError') {
            // 인증 취소 또는 iOS에서 패스키/보안키 미지원 → 전화번호 입력으로 전환
            msgEl.innerHTML = '<div class="msg msg-info">인증이 취소되었습니다. 전화번호로 진행해주세요.</div>';
            setTimeout(function() { showStep(1); }, 1500);
      ...
      async function registerBiometric() {
        ...
        try {
          // 0. 등록 토큰 발급 (phone-lookup → reg_token 생성)
          if (!currentRegToken) {
            const tokenRes = await fetch('/api/register/phone-lookup', {
              method: 'POST',
              headers: { 'Content-Type': 'application/json' },
            ...
            if (!tokenData.found || !tokenData.hasToken) {
              throw new Error(tokenData.error || '등록 토큰 발급에 실패했습니다. 관리자에게 문의하세요.');
            }
