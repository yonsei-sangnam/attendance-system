...
app.get('/app-auth', async (req, res) => {
  ...
  var html = '<!DOCTYPE html>'
    ...
    + '  catch (e) { return null; }'
    + '}'
    + 'function withSignal(opts, ctrl) {'
    + '  if (ctrl && ctrl.signal) opts.signal = ctrl.signal;'
    + '  return opts;'
    + '}'
    + 'function withBioTimeout(p, ms, ctrl) {'
    ...
    + '      var err = new Error(\"BIO_TIMEOUT\");'
    + '      err.name = \"BioTimeoutError\";'
    + '      reject(err);'
    + '    }, ms);'
    ...
    + '  });'
    + '}'
    + 'var BIO_TIMEOUT_MSG = \"생체인증 창이 열리지 않았습니다.<br><br>'
    +   '1. 휴대폰 화면 잠금(지문·PIN·패턴)이 설정되어 있는지 확인해 주세요.<br>'
    +   '2. 이 휴대폰에서 생체인증을 등록한 적이 있는지 확인해 주세요.<br>'
    +   '&nbsp;&nbsp;&nbsp;다른 휴대폰에서 등록했다면 이 기기로 재등록이 필요합니다.<br><br>'
    +   '해결되지 않으면 관리자에게 문의해 주세요.\";'
    + ''
    ...
    + ''
    + '    var regCtrl = makeCtrl();'
    + '    var credential = await withBioTimeout('
    +   'navigator.credentials.create(withSignal({ publicKey: options }, regCtrl)), BIO_TIMEOUT_MS, regCtrl);'
    + ''
    ...
    + '        regTransports = credential.response.getTransports() || [];'
    + '      }'
    + '    } catch (e) { regTransports = []; }'
    + '    if (!regTransports.length) regTransports = ["internal"];'
    + ''
    + '    var response = {'
    + '      id: credential.id,'
    ...
    + '    document.getElementById("sub").textContent = "얼굴인식 또는 지문을 사용합니다";'
    + ''
    + '    var authCtrl = makeCtrl();'
    + '    var credential = await withBioTimeout('
    +   'navigator.credentials.get(withSignal({ publicKey: options }, authCtrl)), BIO_TIMEOUT_MS, authCtrl);'
    + ''
...
app.get('/correction', (req, res) => {
  ...
  var html = '<!DOCTYPE html><html lang="ko"><head>'
    ...
    /* 앱(HistoryScreen)과 동일한 디자인 토큰 */
    + 'body{margin:0;padding:0;background:#ecebe9;color:#101112;line-height:1.6;'
    + '  font-family:-apple-system,BlinkMacSystemFont,"Apple SD Gothic Neo","Malgun Gothic",sans-serif;}'
    + '.wrap{max-width:560px;margin:0 auto;padding:12px 0 40px;}'
    + '.header{padding:12px 20px 22px;}'
    + '.h-title{font-size:34px;font-weight:800;color:#101112;letter-spacing:-0.8px;line-height:1.1;}'
    + '.h-sub{font-size:34px;font-weight:600;color:#5E89C6;letter-spacing:-0.8px;line-height:1.1;}'
    + '.h-lead{font-size:13px;color:rgba(16,17,18,0.45);margin-top:10px;font-weight:500;}'
    + '.kicker{font-size:11px;font-weight:700;letter-spacing:0.8px;color:rgba(16,17,18,0.4);}'
    + '.card{background:#fff;border-radius:28px;padding:24px;margin:0 20px 12px;'
    + '  box-shadow:0 2px 12px rgba(0,0,0,0.06);}'
    + '.btn{display:block;width:100%;border:0;border-radius:22px;padding:0;height:54px;'
    + '  font-size:15px;font-weight:800;cursor:pointer;font-family:inherit;letter-spacing:-0.2px;}'
    + '.btn-main{background:#003876;color:#fff;}'
    + '.btn-sub{background:#eceef1;color:#101112;margin-top:10px;}'
    + '.btn:disabled{opacity:.5;}'
    /* 출결 상태 카드 — 앱의 세션 내역 행과 동일한 배색 */
    + '.rec{border-radius:22px;padding:18px 20px;margin-top:8px;cursor:pointer;}'
    + '.rec-row{display:flex;align-items:center;}'
    + '.rec-main{flex:1;min-width:0;}'
    + '.rec-status{font-size:16px;font-weight:800;letter-spacing:-0.2px;}'
    + '.rec-meta{font-size:12px;font-weight:600;margin-top:3px;opacity:.65;}'
    + '.rec-time{font-size:13px;font-weight:700;margin-left:8px;opacity:.8;white-space:nowrap;}'
    + '.rec-tags{margin-top:10px;}'
    ...
    /* iOS Safari 의 time 입력은 기본 스타일이 남으면 폭 계산을 무시해 화면 밖으로 넘친다 */
    + 'input[type=time],input[type=tel],textarea{display:block;width:100%;min-width:0;max-width:100%;'
    + '  -webkit-appearance:none;appearance:none;'
    + '  border:1.5px solid #e3e6ea;border-radius:16px;padding:14px;'
    + '  font-size:16px;font-family:inherit;background:#fff;color:#101112;}'
    + 'input[type=time]{height:52px;}'
    + 'input[type=time]::-webkit-date-and-time-value{text-align:left;margin:0;}'
    + 'textarea{min-height:110px;resize:vertical;line-height:1.6;}'
    + 'input:focus,textarea:focus{outline:none;border-color:#003876;}'
    + '.msg{border-radius:16px;padding:14px;font-size:13.5px;font-weight:700;'
    + '  margin-top:12px;text-align:center;line-height:1.7;}'
    + '.msg-err{background:rgba(211,47,47,0.10);color:#D32F2F;}'
    + '.msg-ok{background:rgba(0,56,118,0.08);color:#003876;}'
    + '.note{font-size:12px;color:rgba(16,17,18,0.4);margin-top:14px;line-height:1.8;}'
    + '.hide{display:none;}'
    + '</style></head><body><div class="wrap">'
    + '<div class="header">'
    +   '<div class="h-title">출결 소명</div>'
    +   '<div class="h-sub">정정 요청</div>'
    +   '<div class="h-lead">출결이 잘못 기록된 경우 사유를 제출하면 관리자가 검토합니다.</div>'
    + '</div>'

    + '<div class="card" id="stepLogin">'
    +   '<div class="kicker">본인 확인</div>'
    ...
    +   '<div class="kicker" id="helloKicker">세션 내역</div>'
    +   '<div style="font-size:17px;font-weight:800;margin-top:6px;letter-spacing:-0.3px;" id="hello"></div>'
    ...
    +   '<div class="kicker">소명 작성</div>'
    +   '<div style="font-size:17px;font-weight:800;margin-top:6px;letter-spacing:-0.3px;" id="formTitle"></div>'
    ...
    + '<div class="note" id="footNote" style="margin:16px 20px 0;">'
    ...
    + ''
    /* 앱 HistoryScreen 의 STATUS 색상표와 동일하게 맞춘다 */
    + 'var DAYS = ["일","월","화","수","목","금","토"];'
    + 'var STATUS = {'
    + '  "출석":      { bg:"#dce8f5", text:"#003876" },'
    + '  "지각":      { bg:"#fbe4d5", text:"#D4760A" },'
    + '  "조퇴":      { bg:"#eae1f8", text:"#7C3AED" },'
    + '  "결석":      { bg:"#fadedd", text:"#D32F2F" },'
    + '  "퇴실미확인": { bg:"#e8e8e8", text:"#757575" }'
    + '};'
    + 'function dispStatus(r){ return r.exit_type === "퇴실미확인" ? "퇴실미확인" : (r.status || "—"); }'
    + 'function statusColor(r){ return STATUS[dispStatus(r)] || STATUS["퇴실미확인"]; }'
    + 'function fmtDate(v){'
    + '  if (!v) return "";'
    + '  var p = String(v).substring(0,10).split("-");'
    + '  if (p.length !== 3) return "";'
    + '  var dt = new Date(p.join("-") + "T12:00:00");'
    + '  var dn = isNaN(dt.getTime()) ? "" : DAYS[dt.getDay()];'
    + '  return p[1] + "." + p[2] + " (" + dn + ")";'
    + '}'
    + 'function fmtTime(t){'
    + '  if (!t) return "—";'
    ...
    + '      var inAt = x.check_in_at ? String(x.check_in_at).slice(11,16) : null;'
    + '      return "<div class=\\"rec\\" data-i=\\"" + i + "\\" style=\\"background:" + sc.bg + ";\\">"'
    + '        + "<div class=\\"rec-row\\"><div class=\\"rec-main\\">"'
    + '        + "<div class=\\"rec-status\\" style=\\"color:" + sc.text + ";\\">" + esc(dispStatus(x)) + "</div>"'
    + '        + "<div class=\\"rec-meta\\" style=\\"color:" + sc.text + ";\\">"'
    + '        + esc(x.course_name) + " · " + esc(x.session_number) + "회차 · " + fmtDate(x.session_date) + "</div>"'
    + '        + "</div>"'
    + '        + "<div class=\\"rec-time\\" style=\\"color:" + sc.text + ";\\">" + (inAt ? fmtTime(inAt) : "—") + "</div>"'
    + '        + "</div>"'
